...
def read_original_data():
    ...
    # 打印出剩余评论的数量。
    print("left comments length is ", len(df))
...
def group_by_kmeans(df_embedded):
...
def embedding_text(client, text, model="text-embedding-ada-002"):
...
def search_reviews(df, product_description, n=3, pprint=True):
    client = OpenAI()
    product_embedding = embedding_text(client, product_description)
    ...
    # search_reviews(df_embedded, 'delicious beans', n=3)


# make this python file runnable
if __name__ == "__main__":
    embedding_data = load_embedding_data()
    search_reviews(embedding_data, 'delicious beans', n=3)
    input("type to exit")
